...
function detailedKoreanPron(ipa){
 let raw=String(ipa||"").trim();if(!raw)return "";
 raw=raw.split(/[;,]/)[0].replace(/[\/\[\]()]/g,"").replace(/:/g,"ː").trim();
 ...
 const onset={"p":"ㅍ","b":"ㅂ","t":"ㅌ","d":"ㄷ","k":"ㅋ","g":"ㄱ","f":"ㅍ","v":"ㅂ","θ":"ㅆ","ð":"ㄷ","s":"ㅅ","z":"ㅈ","ʃ":"ㅅ","ʒ":"ㅈ","h":"ㅎ","tʃ":"ㅊ","dʒ":"ㅈ","m":"ㅁ","n":"ㄴ","ŋ":"ㅇ","l":"ㄹ","r":"ㄹ","ɹ":"ㄹ"};
 ...
 function syl(o,v,c=""){o=(o in lead)?o:"ㅇ";c=(c in tail)?c:"";if(!(v in med))return v;return String.fromCharCode(0xAC00+(lead[o]*21+med[v])*28+tail[c])}
 function tokenise(word){
  const out=[];for(let i=0;i<word.length;){let hit="";for(const m of multi){if(word.startsWith(m,i)){hit=m;break}}if(hit){out.push(hit);i+=hit.length;continue}const ch=word[i];if(consonants.has(ch)||vowels[ch])out.push(ch);i++}
  // American-style flap: vowel + t/d + weak vowel => ㄹ-like sound (water, better, city)
  for(let i=1;i<out.length-1;i++){if((out[i]==="t"||out[i]==="d")&&vowels[out[i-1]]&&vowels[out[i+1]]&&["ə","ɚ","ɪ","i"].includes(out[i+1]))out[i]="r"}
  return out
 }
 function glideVowel(glide,v){if(glide==="j"){const first=v[0];const m={"아":"야","애":"얘","어":"여","에":"예","오":"요","우":"유","이":"이"}[first]||first;return [m,...v.slice(1)]}if(glide==="w"){const first=v[0];const m={"아":"와","애":"왜","어":"워","에":"웨","오":"워","우":"우","이":"위"}[first]||first;return [m,...v.slice(1)]}return v}
 function renderSyllable(onsetTok,vowelSeq,codaTok){let seq=[...vowelSeq],o="ㅇ";if(onsetTok){if(Array.isArray(onsetTok)){const base=onsetTok[0],gl=onsetTok[1];o=onset[base]||"ㅇ";seq=glideVowel(gl,seq)}else if(onsetTok==="j"||onsetTok==="w"){seq=glideVowel(onsetTok,seq)}else o=onset[onsetTok]||"ㅇ"}let txt="";for(let q=0;q<seq.length;q++)txt+=syl(q===0?o:"ㅇ",seq[q],q===seq.length-1&&codaTok?(coda[codaTok]||""):"");return txt}
 function epenthetic(c){if(c==="r"||c==="ɹ")return "ㄹ";if(c==="j")return "이";if(c==="w")return "우";if(c==="t")return "ㅌ";if(c==="p")return "ㅍ";if(c==="k")return "ㅋ";if(c==="f")return "ㅍ";if(c==="v")return "ㅂ";if(c==="θ")return "ㅆ";return syl(onset[c]||"ㅇ","으")}
 function convertWord(word){
  const t=tokenise(word);if(!t.length)return "";const vp=[];for(let i=0;i<t.length;i++)if(vowels[t[i]])vp.push(i);if(!vp.length)return "";const syll=[];
  for(let vi=0;vi<vp.length;vi++){
   const vpos=vp[vi],prevV=vi?vp[vi-1]:-1,nextV=vi+1<vp.length?vp[vi+1]:t.length;let before=t.slice(prevV+1,vpos).filter(x=>consonants.has(x));let after=t.slice(vpos+1,nextV).filter(x=>consonants.has(x));let onsetTok=null,codaTok=null,prefix="",suffix="";
   if(before.length){if(before.length>=2&&(before.at(-1)==="j"||before.at(-1)==="w")){const base=before.at(-2);onsetTok=[base,before.at(-1)];before=before.slice(0,-2)}else{onsetTok=before.pop()}if(vi===0){for(const c of before)prefix+=epenthetic(c)}}
   if(vi===vp.length-1){let finals=after.filter(c=>c!=="r"&&c!=="ɹ");if(after.includes("r")||after.includes("ɹ"))suffix+="ㄹ";
    if(finals.length===1){codaTok=finals[0];if(["t","p","k","f","v","θ"].includes(finals[0]))suffix+=epenthetic(finals[0])}
    else if(finals.length>=2){const pair=finals.slice(-2).join("");if(["nt","nd","mp","ŋk","ld"].includes(pair)){codaTok=finals.at(-2);suffix+=epenthetic(finals.at(-1));finals=finals.slice(0,-2)}else if(pair==="st"){suffix+="스ㅌ";finals=finals.slice(0,-2)}else{codaTok=finals.shift();for(const c of finals)suffix+=epenthetic(c);finals=[]}}
   }else if(after.length>=2){let cluster=[...after];if(cluster.at(-1)==="j"||cluster.at(-1)==="w")cluster=cluster.slice(0,-2);else cluster=cluster.slice(0,-1);cluster=cluster.filter(c=>c!=="r"&&c!=="ɹ");if(cluster.length){codaTok=cluster.shift();for(const c of cluster)suffix+=epenthetic(c)}}
   let piece=prefix+renderSyllable(onsetTok,vowels[t[vpos]],codaTok)+suffix;if(t[vpos]==="ɚ"||t[vpos]==="ɝ")piece+="ㄹ";syll.push(piece)
  }
  return syll.join("")
 }
...
function setQuestion(x){
 ...
 $("word").textContent=x.term;$("pron").textContent=fullPron(x.pron);$("badge").textContent=CAT_META[x.cat]?.[0]||"오답 복습";
...
function showLearn(){
 $("learn").classList.remove("hidden");$("core").innerHTML=`<b>${current.term}</b> <span class="small">${fullPron(current.pron)}</span> = ${current.meaning}`;
...
async function openWrongBook(){
 ...
 else rows.sort((a,b)=>b.last-a.last).forEach(r=>{const d=document.createElement("div");d.className="wrongItem";d.innerHTML=`<strong>${r.term} <span class="small">${fullPron(r.pron)}</span></strong><span>${r.meaning} · 오답 ${r.count}회</span>`;box.appendChild(d)});
 show("wrongBook")
}
